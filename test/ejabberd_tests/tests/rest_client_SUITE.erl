...
messages_can_be_paginated_in_room(Config) ->
    escalus:fresh_story(Config, [{alice, 1}, {bob, 1}], fun(Alice, Bob) ->
        ...
        [_, _, _] = Msgs3,
        OSTime = os:timestamp(),
        {_, Time} = calendar:now_to_datetime(OSTime),
        PriorTo = rest_helper:make_timestamp(-1, Time) - 10000,
        ct:pal("now: ~p", [calendar:now_to_datetime(OSTime)]),
        ct:pal("local: ~p", [calendar:now_to_local_time(OSTime)]),
        ct:pal("univ:: ~p", [calendar:now_to_universal_time(OSTime)]),
