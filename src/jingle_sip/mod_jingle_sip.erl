...
maybe_iq_to_other_user(Acc) ->
    ...
    #jid{luser = LUser} = mongoose_acc:get(origin_jid, Acc),
    case LUser of
        StanzaTo ->
...
translate_to_sip(<<"session-accept">>, Jingle, Acc) ->
    Server = mongoose_acc:get(server, Acc),
    SID = exml_query:attr(Jingle, <<"sid">>),
    case mod_jingle_sip_backend:get_incoming_request(SID, mongoose_acc:get(origin_jid, Acc)) of
        {ok, ReqID} ->
...
translate_to_sip(<<"transport-info">>, Jingle, Acc) ->
    ...
    case mod_jingle_sip_backend:get_outgoing_handle(SID, mongoose_acc:get(origin_jid, Acc)) of
        {ok, undefined} ->
...
translate_to_sip(<<"session-terminate">>, Jingle, Acc) ->
    ...
    From = mongoose_acc:get(origin_jid, Acc),
